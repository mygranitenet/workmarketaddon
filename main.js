(async function() {
    'use strict';
    const SCRIPT_PREFIX = '[WM TRANSFORMER V13-RERUN]';
    ...
    const customCss = `
        ...
        .tech-modal-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #eee; padding-bottom: 10px; margin-bottom: 15px; cursor: move; }
    ...
    function modifyPageSizeSelectOnce(scriptPrefix) {
        ...
        if (pageSizeSelect) {
            // No dataset check needed if we always want to re-run this part
            ...
            // pageSizeSelect.dataset.modifiedByTransformer = 'true'; // No longer needed if we always re-run
...
class WorkMarketTransformer {
    constructor() {
        ...
        this.transformationInitialized = false; // Still useful to prevent multiple observer callbacks from one load
        this.observer = null;
    ...
    async fetchWorkerData(assignmentId) {
        ...
        try {
            ...
            const appliedWorkers = allFetchedWorkers.filter(w => {
                ...
                // From provided JSON, `w.status === "open"` means the invitation/routing is open,
                // and `has_negotiation` implies they've interacted (applied/countered).
    ...
    showTechDetailsModal(techFullDataWithScores, assignmentIdForModal, techIndexInAssignment) {
        ...
        const priorityFields = [ /* ... (Same as V9.FULL_FIX) ... */
        ...
        const renderKeyValuePair = (key, value, parentEl, isNested = false, path = '') => { /* ... (Same as V9.FULL_FIX) ... */
            const formattedValue = this.formatValue(value, key);
            if (formattedValue === 'N/A' && !(key === 'declined_on' && value === '')) return;
            const dt = document.createElement('dt');
            const priorityField = priorityFields.find(pf => pf.key === key);
            dt.textContent = (priorityField?.label || key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())) + ':';
            ...
            else if (key === 'graniteTicket' && value) { dd.innerHTML = `<a href="YOUR_TICKET_SYSTEM_BASE_URL/${value}" target="_blank">${value}</a>`; /* REPLACE YOUR_TICKET_SYSTEM_BASE_URL */ }
            else { dd.textContent = formattedValue; }
            parentEl.appendChild(dt); parentEl.appendChild(dd);
        };
        const renderSection = (title, dataObject, parentEl, isTopLevelSection = true) => { /* ... (Same as V9.FULL_FIX, ensure 'this' context if calling this.formatValue or renderKeyValuePair from here) ... */
            ...
            for (const key in dataObject) { if (dataObject.hasOwnProperty(key)) {
                if ((title.includes('Scorecard (For Your Company)') || title.includes('Scorecard (Overall Platform)')) && typeof dataObject[key] === 'object' && dataObject[key] !== null && dataObject[key].hasOwnProperty('all')) { renderKeyValuePair(key, dataObject[key].all, parentEl, true, title + '.' + key); }
                else if (title.includes('Pricing') || title === 'Negotiation Details' || title.includes('Rating Details') || (title.includes('Scorecard') && typeof dataObject[key] === 'object' && dataObject[key] !== null) ) { if (typeof dataObject[key] !== 'object' || dataObject[key] === null) { renderKeyValuePair(key, dataObject[key], parentEl, true, title + '.' + key); } else { renderSection(key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()), dataObject[key], parentEl, false); } }
                else if (typeof dataObject[key] !== 'object' || dataObject[key] === null) { renderKeyValuePair(key, dataObject[key], parentEl, true, title + '.' + key); }
            } }
        };

        priorityFields.forEach(pf => { if (techRawData.hasOwnProperty(pf.key)) { renderKeyValuePair.call(this, pf.key, techRawData[pf.key], detailsGrid, false, pf.key); } }); // Ensure 'this' context
        if (techRawData.has_negotiation && techRawData.negotiation) { renderSection.call(this, 'Negotiation Details', techRawData.negotiation, detailsGrid); } // Ensure 'this' context
        if (techRawData.resource_scorecard_for_company && techRawData.resource_scorecard_for_company.values) { renderSection.call(this, 'Scorecard (For Your Company)', techRawData.resource_scorecard_for_company.values, detailsGrid); if(techRawData.resource_scorecard_for_company.rating){renderSection.call(this, 'Company Rating Details', techRawData.resource_scorecard_for_company.rating, detailsGrid);} } // Ensure 'this' context
        if (techRawData.resource_scorecard && techRawData.resource_scorecard.values) { renderSection.call(this, 'Scorecard (Overall Platform)', techRawData.resource_scorecard.values, detailsGrid); if(techRawData.resource_scorecard.rating){renderSection.call(this, 'Overall Rating Details', techRawData.resource_scorecard.rating, detailsGrid);} } // Ensure 'this' context
        const keysToExclude = [ /* ... (Same as V9.FULL_FIX) ... */ 'avatar_uri', 'avatar_asset_uri', 'user_uuid', 'encrypted_id', 'valuesWithStringKey', 'tieredPricingMetaData', 'labels', 'dispatcher', 'resource_scorecard_for_company', 'resource_scorecard', 'OverallScore', 'CostScore', 'DistanceScore', 'StatsScore', 'CPS_Final', 'IPS', 'assignmentId', 'raw_worker_data', 'user_id', 'user_number', 'latitude', 'longitude', 'new_user', 'rating_text', 'company_rating_text', 'lane', 'assign_to_first_to_accept', 'blocked', 'name', 'company_name', 'email', 'work_phone', 'mobile_phone', 'address', 'distance', 'status', 'sent_on', 'declined_on', 'question_pending', 'has_negotiation', 'schedule_conflict', 'negotiation', 'targeted'];
        let hasOtherDetails = false; const otherDetailsFragment = document.createDocumentFragment();
        for (const key in techRawData) { if (techRawData.hasOwnProperty(key) && !keysToExclude.includes(key) && !priorityFields.find(pf => pf.key === key)) { const value = techRawData[key]; if (value !== null && value !== undefined && String(value).trim() !== '') { if(!hasOtherDetails){ const otherDt = document.createElement('dt'); otherDt.className = 'section-header-dt'; otherDt.textContent = 'Other Raw Details'; otherDetailsFragment.appendChild(otherDt); hasOtherDetails = true; } renderKeyValuePair.call(this, key, value, otherDetailsFragment, false, key); } } } // Ensure 'this' context
    ...
    async initializeTransformationSequence() { /* ... (Same as V9.FULL_FIX, ensure `this` is used for method calls) ... */
        ...
        this.sortData(); this.renderTable(this.tableData, this.activeTableHeaders, this.mainOverlayContentTarget); console.log(`${this.SCRIPT_PREFIX} All transformations complete. Final table rendered in overlay with ${this.tableData.length} assignments.`);
    }
...
// --- Script Entry Point ---
try {
    // We remove the check for window.WorkMarketTransformerInstance to allow re-injection and re-run
    // However, be mindful of creating multiple overlays or modal structures if not handled carefully.
    // The createMainOverlay and createTechModal methods have checks to prevent duplicate DOM elements.
    ...
    if (document.getElementById('assignment_list_results') &&
        ...
        // Call global setup functions ONCE per page load (or ensure they are idempotent)
        ...
        // If an old instance's observer is running, try to disconnect it.
        ...
        // Remove old overlay if it exists to allow new one to be cleanly created by new instance
