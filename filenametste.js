(async function() {
    'use strict';
    const SCRIPT_PREFIX = '[WM TRANSFORMER V11.3-IMMEDIATE_FULL]';
    ...
    const customCss = `
        ...
        .tech-modal-header { display: flex; justify-content: space-between; align-items: center; border-bottom: 1px solid #eee; padding-bottom: 10px; margin-bottom: 15px; cursor: move; } /* Added cursor move to header */
...
class WorkMarketTransformer {
    constructor() {
        ...
        console.log(`${this.SCRIPT_PREFIX} Constructor finished. Attempting IMMEDIATE transformation...`);
        Promise.resolve().then(() => {
            this.initializeTransformationSequence().catch(err => {
                console.error(`${this.SCRIPT_PREFIX} Error during DIRECT initializeTransformationSequence:`, err);
            });
        });
    }
    ...
    async fetchWorkerData(assignmentId) {
        ...
        try {
            ...
            const appliedWorkers = allFetchedWorkers.filter(w => {
                ...
                // CRITICAL: Update this 'hasApplied' logic based on your specific API data.
                const hasApplied = hasActiveNegotiation; // Current best guess for "applied"
    ...
    showTechDetailsModal(techFullDataWithScores, assignmentIdForModal, techIndexInAssignment) { /* ... (Same as V10 - recursive renderObject for modal) ... */ }
    ...
    waitForAssignmentsAndInitialize() {
        ...
        let observer = new MutationObserver((mutationsList, obs) => { if (this.originalResultsContainerSource.querySelector(this.assignmentItemSelector)) { console.log(`${this.SCRIPT_PREFIX} Assignment items DETECTED by MutationObserver.`); obs.disconnect(); if (!this.transformationInitialized) { this.transformationInitialized = true; this.initializeTransformationSequence().catch(err => console.error(`${this.SCRIPT_PREFIX} Error (observer init):`, err)); } } });
        try { observer.observe(this.originalResultsContainerSource, { childList: true, subtree: true }); console.log(`${this.SCRIPT_PREFIX} MutationObserver started.`); }
        catch (e) { console.error(`${this.SCRIPT_PREFIX} ERROR starting MutationObserver:`, e); observer = null; this.attemptFallbackInitializationPolling(null); return; }
        setTimeout(() => { this.attemptFallbackInitializationPolling(observer); }, 1500);
    }

    attemptFallbackInitializationPolling(observerInstance) { // Renamed for clarity
        let pollAttempts = 0;
        const maxPollAttempts = 20; const pollInterval = 500;
        const pollForItems = () => {
            if (this.transformationInitialized) { if (observerInstance) observerInstance.disconnect(); return; }
            pollAttempts++; console.log(`${this.SCRIPT_PREFIX} Polling attempt #${pollAttempts} for assignment items...`);
            if (this.originalResultsContainerSource.querySelector(this.assignmentItemSelector)) {
        ...
        if (!this.transformationInitialized) { // Only start polling if not already done
           setTimeout(pollForItems, pollInterval); // Start after a small delay
        } else if (observerInstance) {
            observerInstance.disconnect(); // If already initialized by observer, ensure it's stopped.
        }
    }

    async initializeTransformationSequence() {
        console.log(`${this.SCRIPT_PREFIX} Starting main transformation sequence (IMMEDIATE CALL OR POST-WAIT)...`);
