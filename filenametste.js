(async function() {
    'use strict';
    const SCRIPT_PREFIX = '[WM TRANSFORMER V12-MO-REFINED]';
...
class WorkMarketTransformer {
    constructor() {
        ...
        this.waitForAssignmentsAndInitialize(); // This is the sole entry point now
    }
    ...
    async fetchWorkerData(assignmentId) {
        ...
        try {
            ...
            const appliedWorkers = allFetchedWorkers.filter(w => {
                ...
                // ** CRITICAL: Based on your JSON sample, this is the most likely "applied" condition. **
                // If `w.status` provides a more direct 'APPLIED' state, incorporate it.
                // e.g., const isExplicitlyApplied = w.status === 'APPLIED_BY_WORKER';
                // const hasApplied = isExplicitlyApplied || hasActiveNegotiation;
    ...
    showTechDetailsModal(techFullDataWithScores, assignmentIdForModal, techIndexInAssignment) {
        ...
        const renderKeyValuePair = (key, value, parentEl, isNested = false) => {
            ...
            const dt = document.createElement('dt'); dt.textContent = (key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())) + ':';
        ...
        const renderSection = (title, dataObject, parentEl, isNestedForChildren = false) => {
            ...
            headerDt.className = title.includes('Pricing') || title.includes('Rating') ? 'sub-section-dt' : 'section-header-dt';
            ...
            for (const key in dataObject) {
                if (dataObject.hasOwnProperty(key)) {
                    ...
                    } else if (title.includes('Pricing') || title === 'Negotiation Details') {
        ...
        priorityFields.forEach(pf => { if (techRawData.hasOwnProperty(pf.key)) { renderKeyValuePair(pf.key, techRawData[pf.key], detailsGrid, false); } });
        if (techRawData.has_negotiation && techRawData.negotiation) { const negHeaderDt = document.createElement('dt'); negHeaderDt.className = 'section-header-dt'; negHeaderDt.textContent = 'Negotiation Details'; detailsGrid.appendChild(negHeaderDt); for (const negKey in techRawData.negotiation) { if (techRawData.negotiation.hasOwnProperty(negKey) && negKey !== 'pricing') { renderKeyValuePair(negKey, techRawData.negotiation[negKey], detailsGrid, true); } } if (techRawData.negotiation.pricing) { renderSection('Pricing', techRawData.negotiation.pricing, detailsGrid); } }
        if (techRawData.resource_scorecard_for_company && techRawData.resource_scorecard_for_company.values) { renderSection('Scorecard (For Your Company)', techRawData.resource_scorecard_for_company.values, detailsGrid); }
        if (techRawData.resource_scorecard && techRawData.resource_scorecard.values) { renderSection('Scorecard (Overall Platform)', techRawData.resource_scorecard.values, detailsGrid); }
        const keysToExclude = ['avatar_uri', 'avatar_asset_uri', 'user_uuid', 'encrypted_id', 'valuesWithStringKey', 'tieredPricingMetaData', 'labels', 'dispatcher', 'resource_scorecard_for_company', 'resource_scorecard', 'OverallScore', 'CostScore', 'DistanceScore', 'StatsScore', 'CPS_Final', 'IPS', 'assignmentId', 'raw_worker_data', 'user_id', 'user_number', 'latitude', 'longitude', 'new_user', 'rating_text', 'company_rating_text', 'lane', 'assign_to_first_to_accept', 'blocked', 'name', 'company_name', 'email', 'work_phone', 'mobile_phone', 'address', 'distance', 'status', 'sent_on', 'declined_on', 'question_pending', 'has_negotiation', 'schedule_conflict', 'negotiation'];
        let hasOtherDetails = false;
        const otherDetailsContainer = document.createDocumentFragment(); // Create a fragment to append "Other Details" together
        for (const key in techRawData) {
            if (techRawData.hasOwnProperty(key) && !keysToExclude.includes(key) && !priorityFields.find(pf => pf.key === key)) {
                const value = techRawData[key];
                const formattedValue = this.formatValue(value,key);
                if(formattedValue !== 'N/A'){
                    if(!hasOtherDetails){
                        const otherDetailsHeader = document.createElement('dt'); otherDetailsHeader.className = 'section-header-dt'; otherDetailsHeader.textContent = 'Other Raw Details';
                        otherDetailsContainer.appendChild(otherDetailsHeader); // Add header to fragment
                        hasOtherDetails = true;
                    }
                    renderKeyValuePair(key, value, otherDetailsContainer, false); // Render to fragment
                }
            }
        }
        if(hasOtherDetails) detailsGrid.appendChild(otherDetailsContainer); // Append fragment if it has content
    ...
    waitForAssignmentsAndInitialize() {
        ...
        if (this.transformationInitialized) { console.log(`${this.SCRIPT_PREFIX} waitForAssignmentsAndInitialize: Transformation already ran. Exiting.`); return; }
        if (!this.originalResultsContainerSource) { console.error(`${this.SCRIPT_PREFIX} waitForAssignmentsAndInitialize: originalResultsContainerSource is null!`); return; }
        ...
        setTimeout(() => { if (!this.transformationInitialized) { this.attemptFallbackInitializationPolling(this.observer); } }, 2000); // Fallback polling starts after 2s
    }
    ...
    attemptFallbackInitializationPolling(observerInstance) {
        ...
        pollForItems(); // Start the first poll immediately from here
    }
    ...
    async initializeTransformationSequence() {
        ...
        console.log(`${this.SCRIPT_PREFIX} Skeleton render to overlay complete.`);
