(async function() {
    'use strict';
    const SCRIPT_PREFIX = '[WM TRANSFORMER V12.FINAL_FULL_CODE]';
    ...
    /**
     * Global function to inject CSS styles once.
     */
    ...
    /**
     * Global function to modify the page size select dropdown once.
     */
    function modifyPageSizeSelectOnce(scriptPrefix) {
        ...
        if (pageSizeSelect) {
            ...
            if (!isCurrentSelectedStillAvailable && pageSizeSelect.options.length > 0) { /* Optional: select a default */ }
...
class WorkMarketTransformer {
    ...
    async fetchWorkerData(assignmentId) {
        ...
        // console.log(`${this.SCRIPT_PREFIX} API Call [START] - Fetching workers for Assignment ID: ${assignmentId}`);
        let responseText = '';
        try {
            const response = await fetch(url, { headers: { 'Accept': 'application/json, text/plain, */*', 'X-Requested-With': 'XMLHttpRequest' } });
            responseText = await response.text();
            if (!response.ok) {
                console.error(`${this.SCRIPT_PREFIX} API Call [ERROR] - Failed for ${assignmentId}: ${response.status} ${response.statusText}. Response Text:`, responseText);
                return { count: 0, applicantDetailsDisplay: `Error ${response.status}`, top10TechsFullData: [] };
            }
            const data = JSON.parse(responseText);
            // console.log(`${this.SCRIPT_PREFIX} API Call [SUCCESS RAW JSON] - Worker data for ${assignmentId}:`, JSON.parse(JSON.stringify(data)));

            let allFetchedWorkers = data.results || [];
            const totalFetchedInitially = allFetchedWorkers.length;

            // console.log(`${this.SCRIPT_PREFIX} DEBUG: Examining ${totalFetchedInitially} fetched workers for assignment ${assignmentId} BEFORE filtering.`);
            // for (let i = 0; i < Math.min(1, allFetchedWorkers.length); i++) {
            //     console.log(`${this.SCRIPT_PREFIX} DEBUG: Raw worker object [${i}] for ${assignmentId}:`, JSON.parse(JSON.stringify(allFetchedWorkers[i])));
            // }

            const appliedWorkers = allFetchedWorkers.filter(w => {
                const isNotDeclined = w.declined_on === ""; // True if they haven't declined
                // Based on your sample, `status: "open"` AND `has_negotiation: true` (implying `negotiation` object exists)
                // seems to be the most reliable indicator for an "applied" or actively engaged worker.
                const hasActiveNegotiation = w.has_negotiation === true && w.negotiation !== null;

                // IMPORTANT: Verify this condition based on how your system truly defines "applied".
                // If w.status changes from "open" upon application, or if another field signifies this,
                // adjust the `hasApplied` logic.
                const hasApplied = hasActiveNegotiation && w.status === "open"; // Workers who have interacted AND their invite is still 'open'

                return isNotDeclined && hasApplied;
            });
            // console.log(`${this.SCRIPT_PREFIX} API Call - Total fetched: ${totalFetchedInitially}, Filtered to ${appliedWorkers.length} "applied" & not declined for ${assignmentId}.`);
            ...
            const top10TechsFullData = top10WorkersRaw.map(w => {
                const techWithScore = { ...w, assignmentId: assignmentId }; // Add assignmentId
                const scores = this.calculateOverallScore(techWithScore); // Calculate scores
                return { ...techWithScore, ...scores }; // Merge scores into the tech data
            });

            const listItems = top10WorkersRaw.map((tech, index) => {
                let displayName = '';
                if (tech.company_name && tech.company_name.toLowerCase() === 'sole proprietor') {
                    displayName = tech.name || tech.company_name || 'N/A';
                } else {
                    displayName = tech.company_name || 'N/A';
                    if (tech.name && tech.name.toLowerCase() !== tech.company_name.toLowerCase()) {
    ...
    doDragTechModal(e) { if (!this.techModalIsDragging) return; const modalContent = document.getElementById('techDetailModalOverlay').querySelector('.tech-modal-content'); if (!modalContent) return; modalContent.style.left = (e.clientX - this.techModalDragStartX) + 'px'; modalContent.style.top = (e.clientY - this.techModalDragStartY) + 'px'; } /* Note: dragging the content div, not the overlay */
    ...
    showTechDetailsModal(techFullDataWithScores, assignmentIdForModal, techIndexInAssignment) {
        ...
        const renderKeyValuePair = (key, value, parentEl, isNested = false, path = '') => {
            ...
            if (formattedValue === 'N/A' && !(key === 'declined_on' && value === '')) return; // Show empty declined_on

            const dt = document.createElement('dt');
            dt.textContent = (key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())) + ':';
            if (isNested) dt.style.paddingLeft = '15px'; // Indent only nested keys, not section headers
        ...
        const renderSection = (title, dataObject, parentEl, isTopLevelSection = true) => {
            ...
            for (const key in dataObject) {
                if (dataObject.hasOwnProperty(key)) {
                    if (title.includes('Scorecard') && typeof dataObject[key] === 'object' && dataObject[key] !== null && dataObject[key].hasOwnProperty('all')) {
                         renderKeyValuePair(key, dataObject[key].all, parentEl, true, title + '.' + key);
                    } else if (title.includes('Pricing') || title === 'Negotiation Details' || (title.includes('Scorecard') && !dataObject[key].hasOwnProperty('all'))) { // Render non-'all' scorecard values directly if they are not objects
                         if (typeof dataObject[key] !== 'object' || dataObject[key] === null) {
                            renderKeyValuePair(key, dataObject[key], parentEl, true, title + '.' + key);
                         } else { // It's a nested object within Negotiation/Pricing/Scorecard (e.g. resource_scorecard.rating)
                            renderSection(key.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase()), dataObject[key], parentEl, false); // Not a top level section
                         }
                    }
                }
            }
        ...
        priorityFields.forEach(pf => { if (techRawData.hasOwnProperty(pf.key)) { renderKeyValuePair(pf.key, techRawData[pf.key], detailsGrid, false, pf.key); } });
        if (techRawData.has_negotiation && techRawData.negotiation) {
            renderSection('Negotiation Details', techRawData.negotiation, detailsGrid);
        }
        if (techRawData.resource_scorecard_for_company && techRawData.resource_scorecard_for_company.values) {
            renderSection('Scorecard (For Your Company)', techRawData.resource_scorecard_for_company.values, detailsGrid);
             if(techRawData.resource_scorecard_for_company.rating){renderSection('Company Rating Details', techRawData.resource_scorecard_for_company.rating, detailsGrid);}
        }
        if (techRawData.resource_scorecard && techRawData.resource_scorecard.values) {
            ...
            if(techRawData.resource_scorecard.rating){renderSection('Overall Rating Details', techRawData.resource_scorecard.rating, detailsGrid);}
        }
    ...
    waitForAssignmentsAndInitialize() {
        ...
        if (this.originalResultsContainerSource.querySelector(this.assignmentItemSelector)) {
            console.log(`${this.SCRIPT_PREFIX} Assignment items found on IMMEDIATE check.`);
    ...
    async initializeTransformationSequence() {
        ...
        // console.log(`${this.SCRIPT_PREFIX} Performing initial skeleton render into overlay from ${originalAssignmentNodes.length} nodes...`);
